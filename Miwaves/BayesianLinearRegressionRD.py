...
class BayesianLinearRegressionRD(RLAlgorithm):
    ...
    def __init__(
        ...
        self.inverse_prior_cov = np.linalg.inv(prior_cov)
    ...
    def get_action(self, user: int, decision_idx: int):
        ...
        advantage_with_intercept = np.array(
            advantage_default[: PARAM_SIZE[self.variant][1]]
        )

        # Compute the posterior mean of the adv term
        beta_mean = np.array(self.posterior_mean[-PARAM_SIZE[self.variant][2] :])
    ...
    def update_posteriors(self):
        ...
        self.posterior_cov = np.linalg.inv(
            ...
            + (1 / self.noise_var) * design_matrix.T @ design_matrix
        )
        self.posterior_mean = self.posterior_cov @ (
            np.linalg.inv(self.prior_cov) @ self.prior_mean.reshape(-1, 1)
            + 1 / self.noise_var * design_matrix.T @ reward_matrix.reshape(-1, 1)
        )
    ...
    def update_hyperparameters(self):
        ...
        def obj_func(
            ...
            reward_matrix: jnp.array,
            mu_prior: jnp.array,
            sigma_inv_prior: jnp.array,
            t: int,
            debug: bool = False,
        ):
            """Objective function for optimization"""

            A = jnp.array(design_matrix.T @ design_matrix)
            B = jnp.array(design_matrix.T @ reward_matrix.reshape(-1, 1)).flatten()

            X = jnp.array(sigma_inv_prior)
            mu_prior = jnp.array(mu_prior)

            # y = 1.0 / noise_var
            y = noise_precision

            _, part1 = jnp.linalg.slogdet(X + y * A)
            part1 = -1 * part1
            part2 = t * jnp.log(y)
            part3 = -1 * y * jnp.sum(jnp.square(reward_matrix))
            part4 = (
                (X @ mu_prior + y * B).T
                @ jnp.linalg.inv(X + y * A)
                @ (X @ mu_prior + y * B)
            )
        ...
        old_obj = obj_func(
            ...
            reward_matrix,
            self.prior_mean,
            self.inverse_prior_cov,
            self.num_users * self.current_decision_point,
            debug=False,
        )
        ...
        for idx in range(self.max_iter):
            ...
            grad = jax.grad(obj_func, argnums=0)(
                init_value,
                design_matrix,
                reward_matrix,
                self.prior_mean,
            ...
            if init_value - lr * grad > 0.0001:
                init_value = init_value - lr * grad
            else:
                lr = lr / 2
            ...
            if self.debug:
                print(
                    "Iteration: {}, Noise variance: {}, Objective value: {}".format(
                        idx, 1.0 / init_value, obj_val
                    )
                )
