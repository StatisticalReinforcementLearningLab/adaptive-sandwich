...
parser.add_argument(
    ...
    choices=[RLStudyArgs.EVALSIM, RLStudyArgs.REALISTIC, RLStudyArgs.MEDIUM, RLStudyArgs.EASY],
    help='Sets default parameter values accordingly'
)
parser.add_argument('--synthetic_mode', type=str, default=RLStudyArgs.DELAYED_EFFECTS,
                    help='File name of synthetic env params')
parser.add_argument(
    '--RL_alg', 
    default=RLStudyArgs.SIGMOID_LS,
    choices=[RLStudyArgs.FIXED_RANDOMIZATION, RLStudyArgs.SIGMOID_LS, RLStudyArgs.POSTERIOR_SAMPLING],
...
parser.add_argument('--err_corr', default=RLStudyArgs.TIME_CORR,
...
parser.add_argument('--alg_state_feats', type=str, default=RLStudyArgs.INTERCEPT,
...
parser.add_argument('--prior', type=str, default=RLStudyArgs.NAIVE,
...
if tmp_args.dataset_type == RLStudyArgs.HEARTSTEPS:
    ...
    alg_state_feats = [RLStudyArgs.INTERCEPT, RLStudyArgs.TIME_OF_DAY, RLStudyArgs.PRIOR_DAY_BRUSH]
    alg_treat_feats = alg_state_feats
    
else:
    raise ValueError("Invalid Dataset Type")

parser.add_argument('--T', type=int, default=arg_dict[RLStudyArgs.T],
                    help='Total number of decision times per user')
parser.add_argument('--recruit_n', type=int, default=arg_dict[RLStudyArgs.RECRUIT_N],
                    help='Number of users recruited on each recruitment times')
parser.add_argument('--recruit_t', type=int, default=arg_dict[RLStudyArgs.RECRUIT_T],
                    help='Number of updates between recruitment times (minmum 1)')
parser.add_argument('--allocation_sigma', type=float, 
                    default=arg_dict[RLStudyArgs.ALLOCATION_SIGMA],
                    help='Sigma used in allocation of algorithm')
parser.add_argument('--noise_var', type=float, default=arg_dict[RLStudyArgs.ALLOCATION_SIGMA],
...
def run_study_simulation(study_env, study_RLalg):
    ...
    for t in range(1, study_env.calendar_T+1):
        ...
        if (t % args.decisions_between_updates == 0 and 
                args.RL_alg != RLStudyArgs.FIXED_RANDOMIZATION):
...
if args.dataset_type == RLStudyArgs.ORALYTICS:
    ...
    exp_str = '{}_mode={}_alg={}_T={}_n={}_recruitN={}_decisionsBtwnUpdates={}_steepness={}_algfeats={}_errcorr={}_actionC={}'.format(
            args.dataset_type, mode, args.RL_alg, args.T, args.n, args.recruit_n, 
            args.decisions_between_updates, args.steepness, args.alg_state_feats, args.err_corr, 
...
for i in range(1,args.N+1):
    ...
    if i == 0:
        print(study_df.head())
        if args.dataset_type == RLStudyArgs.HEARTSTEPS and args.mode == RLStudyArgs.EVALSIM:
