...
filename="_01envinference_alpha10.1_alpha20.1"
output_folder = f"/n6_T3/0/simulated_data/synthetic_mode=delayed_1_action_dosage_alg=sac_T=4_n=6_partial{filename}"
...
action_prob_func_filename="functions_to_pass_to_analysis/synthetic_get_action_1_prob_partial.py" # smooth allocation (sigmoid) function 
...
alg_update_func_filename="functions_to_pass_to_analysis/synthetic_SAC_alg_update_function_partial.py"
...
@click.option(
    ...
    default=action_prob_func_filename,
...
@click.option(
    ...
    default=action_prob_func_args_beta_index,
    type=int,
    required=True,
...
@click.option(
    ...
    default=alg_update_func_filename,
    type=click.Path(exists=True),
...
@click.option(
    ...
    default=alg_update_func_args_beta_index,
    type=int,
    required=True,
...
@click.option(
    ...
    default=alg_update_func_args_action_prob_index,
    type=int,
    # default=-1000,
...
@click.option(
    ...
    default=alg_update_func_args_action_prob_times_index,
    type=int,
    # default=-1000,
...
@click.option( ######## important
    ...
    default=inference_func_filename,
    type=click.Path(exists=True),
...
@click.option(
    ...
    default=inference_func_args_theta_index,
    type=int,
    required=True,
...
@click.option( ######## important
    ...
    default=theta_calculation_func_filename,
    type=click.Path(exists=True),
...
@click.option(
    ...
    default=suppress_interactive_data_checks,
    type=bool,
    # default=False,
...
@click.option(
    ...
    default=suppress_all_data_checks,
    type=bool,
    # default=False,
...
def analyze_dataset(
    ...
    alg_update_func_args = pickle.load(alg_update_func_args_pickle)   # it contains the args with values used for evaluating the algorithm estimation function

    # study_df_treat = pickle.load(study_df_pickle_random)
    # study_df_control = pickle.load(study_df_pickle_random)
    
    # study_df = pandas.concat([study_df_treat, study_df_control], ignore_index=True, axis=0)

    # action_prob_func_args_treat = pickle.load(action_prob_func_args_pickle)
    # # action_prob_func_args_control = pickle.load(action_prob_func_args_pickle_random)
    
    # # put all units in each calendar_t together
    # action_prob_func_args = {}
    # for calendar_t in action_prob_func_args_treat.keys():
    #     action_prob_func_args[calendar_t] = action_prob_func_args_treat[calendar_t] | action_prob_func_args_control[calendar_t]

    # alg_update_func_args_treat = pickle.load(alg_update_func_args_pickle)
    # alg_update_func_args_control = pickle.load(alg_update_func_args_pickle_random)

    # alg_update_func_args = {}
    # for calendar_t in alg_update_func_args_treat.keys():
    #     # print('calendar_t',calendar_t)
    #     # print('treat',alg_update_func_args_treat[calendar_t].keys()) # [1, 100]
    #     # print('control',alg_update_func_args_control[calendar_t].keys()) # [101, 200]
    #     alg_update_func_args[calendar_t] = alg_update_func_args_treat[calendar_t] | alg_update_func_args_control[calendar_t]
    #     # alg_update_func_args[calendar_t] = alg_update_func_args_treat[calendar_t] 


    # mean estimate
    theta_est_ = estimate_theta(study_df, theta_calculation_func_filename) # 
    theta_est = jnp.array(theta_est_) # theta_calculation_func(study_df), estimate_theta_avg_reward_sum -> mean of rewards
    ...
    Z_ids = jnp.array(study_df["Z_id"].to_numpy().reshape(user_ids.size,-1)[:,0:1]) # [n, 1]
    (
...
def process_inference_func_args(
    ...
    ##### for each user, collect their inference needed data from the study_df
    ...
    # simple form when no action centering is used
    return (
...
def single_user_weighted_estimating_function_stacker(
    ...
    # for k, v in threaded_inference_func_args.items():
    #     print("key:", k)
    #     print("value:", v)
    #     print("-----")
    # print(type(threaded_inference_func_args))
    # for i, arg in enumerate(threaded_inference_func_args):
    #     print(f"Arg[{i}] = {arg}") # Arg[0] = a scalar
    ...
    num_args = None
    # print("sorted_threaded_action_prob_args_by_decision_time",sorted_threaded_action_prob_args_by_decision_time)
    ...
    batched_threaded_arg_lists = [[] for _ in range(num_args)]
        ...
        # print(sorted_threaded_action_prob_args_by_decision_time)
    # except:
    #     num_args = 6
    #     batched_threaded_arg_lists = [[] for _ in range(num_args)]
        # print("sorted_threaded_action_prob_args_by_decision_time",sorted_threaded_action_prob_args_by_decision_time)
    ...
    ######## complete the implementaion above
    # jax.debug.print("algorithm_component: {}", algorithm_component)
    # jax.debug.print("inference_component: {}", inference_component)
...
def get_avg_weighted_estimating_function_stack_and_aux_values(
    ...
    # define the gradient function (the estimating function) if the supplied function is a loss
    ...
    # print("threaded_inference_func_args_by_user_id:", jnp.asarray(threaded_inference_func_args_by_user_id[1]))
    # print(threaded_inference_func_args_by_user_id[1].keys())
    ...
    results = [
        single_user_weighted_estimating_function_stacker(
            beta_dim,
            user_id,
            action_prob_func, # important   
            algorithm_estimating_func, # important   
            inference_estimating_func, 
            action_prob_func_args_beta_index,
            inference_func_args_theta_index,
            action_prob_func_args_by_decision_time_by_user_id[user_id],
            threaded_action_prob_func_args_by_decision_time_by_user_id[user_id],
            threaded_update_func_args_by_policy_num_by_user_id[user_id], # pareters for algorithm estimation function
            threaded_inference_func_args_by_user_id[user_id],
    ...
    return jnp.mean(stacks, axis=0), (
        ...
        jnp.mean(outer_products, axis=0), # joint_adaptive_meat
