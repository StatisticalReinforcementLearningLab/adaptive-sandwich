...
def output_variance_pieces(study_df, study_RLalg, args, verbose=False):
    ...
    for policy_num, curr_policy_dict in enumerate(study_RLalg.all_policies):
        ...
        est_eqns_dict = study_RLalg.get_est_eqns_full(
        ...
        weighted_pi_grad = study_RLalg.get_pi_gradients(
            curr_timestep_data, curr_policy_dict, verbose
        )
...
def get_adaptive_sandwich(all_est_eqn_dict, args, Teval, alg_correction=""):
    ...
    # Regularization
    final_bread[:-theta_dim, :-theta_dim] = (
        final_bread[:-theta_dim, :-theta_dim]
    ...
    adaptive_sandwich_full = np.matmul(
        np.matmul(final_bread_inv, stacked_meat), final_bread_inv.T
    )
    ...
    return (
        adaptive_sandwich_theta,
        adaptive_sandwich_full,
        eig_dict,
        final_bread,
        stacked_meat,
    )
