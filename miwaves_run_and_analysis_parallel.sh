...
while getopts m:T:s:S:G:t:g:e:O:o:i:c:p:C:U:E:P:b:l:Z:B:D:j:I:h:J:H:Q:q:z:-: OPT; do
...
echo $(date +"%Y-%m-%d %T") miwaves_run_and_analysis_parallel.sh: Loading mamba and CUDA modules.
...
cd ~
if ! test -d venv; then
  echo $(date +"%Y-%m-%d %T") miwaves_run_and_analysis_parallel.sh: Creating venv, as it did not exist.
  python3 -m venv venv
fi
...
echo $(date +"%Y-%m-%d %T") miwaves_run_and_analysis_parallel.sh: Making sure Python requirements are installed.
pip install -r simulation_requirements.txt
echo $(date +"%Y-%m-%d %T") miwaves_run_and_analysis_parallel.sh: All Python requirements installed.
...
echo "$(date +"%Y-%m-%d %T") miwaves_run_and_analysis_parallel.sh: When all jobs have completed, you may collect and summarize the analyses with: bash simulation_collect_analyses.sh --input_glob=${output_folder_glob}/analysis.pkl --index_to_check_ci_coverage=1"
