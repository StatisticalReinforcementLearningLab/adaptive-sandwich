...
def LME_estimating_function(
    ...
    UEt_flat = beta[dim:]
    UEt = jnp.zeros((dim, dim), dtype=jnp.float32).at[jnp.triu_indices(dim)].set(UEt_flat)
    E_t = UEt + UEt.T - jnp.diag(jnp.diag(UEt))

    term1 = sigma_beta_inv @ mu_beta / n_users
    term2 = B_beta / init_noise_var
    term3 = A_beta_gamma @ jnp.linalg.inv(A_gamma + init_noise_var * sigma_gamma_inv) @ B_gamma / init_noise_var
    term4 = E_t @ lambda_t
    ...
    term5 = jnp.triu(
        (sigma_beta_inv / n_users) + (A_beta / init_noise_var) -
        (A_beta_gamma @ jnp.linalg.inv(A_gamma + init_noise_var * sigma_gamma_inv) @ A_beta_gamma.T) / init_noise_var
    )
    term6 = jnp.triu(E_t)
