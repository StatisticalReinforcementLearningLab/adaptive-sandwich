...
def get_stacked_estimating_function(beta_params, beta_dim, all_est_eqn_dict, 
                                    study_RLalg, study_df):
    all_user_ids = np.unique( study_df['user_id'].to_numpy() )
  
    num_updates = int( len(beta_params) / beta_dim )
    beta_params_list = np.split(beta_params, num_updates)
    ...
    for policy_num, curr_policy_dict in enumerate(study_RLalg.all_policies):
        ...
        beta_params = beta_params_list[policy_num-1]
        ...
        est_eqns_dict = study_RLalg.get_est_eqns(beta_params=beta_params,
        ...
        user_pi_weights = study_RLalg.get_weights(curr_policy_decision_data, 
                                                  beta_params, all_user_ids=all_user_ids)
...
def get_adaptive_sandwich_new(all_est_eqn_dict, study_RLalg, study_df, 
    ...
    stacked_est_func = get_stacked_estimating_function(all_est_params, 
                                                       beta_dim, all_est_eqn_dict, 
                                                       study_RLalg, study_df)
...
for i in range(1,args.N+1):
    ...
    get_adaptive_sandwich_new(est_val_dict, study_RLalg, study_df)
