...
class SmoothPosteriorSampling:
    ...
    def __init__(self, args, state_feats, treat_feats, alg_seed, allocation_sigma, 
        ...
        #self.norm_samples =  np.around(norm_samples, PRECISION)
    ...
    def get_est_eqns(self, beta_est, data_dict, info_dict, return_ave_only=False, 
        ...
        if self.action_centering:
            action1prob = data_dict['action1prob'] # TODO may be plugins?
    ...
    def get_weights(self, beta_est, collected_data_dict, curr_policy_decision_data=None,
        ...
        import pickle as pkl
        with open('temp_debug.pkl', 'rb') as file:
            prob_input_dict = pkl.load( file )

        print(prob_input_dict['probs'] == used_prob1)
        print(prob_input_dict['norm_samples'] == collected_data_dict['norm_samples'])
        print(prob_input_dict['treat_states'] == treat_states)
        print(prob_input_dict['beta_est'] == beta_est)
        print(prob_input_dict['n_users'] == collected_data_dict["n_users"])
        print(prob_input_dict['intercept_val'] == collected_data_dict["intercept_val"])
        

        # dict_keys(['treat_states', 'n_users', 'intercept_val', 'norm_samples', 'state_dim', 'beta_est', 'probs'])
        import ipdb; ipdb.set_trace()
        """
    ...
    def get_est_eqns_df(self, beta_est, data_sofar, all_user_ids,
        ...
        if return_ave_only:
            return np.sum(est_eqn_dict['est_eqns'], axis=0) / len(user_ids) # TODO
        return est_eqn_dict
