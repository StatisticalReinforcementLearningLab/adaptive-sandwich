...
def get_stacked_estimating_function(all_estimators, update2esteqn, policy2collected,
    ...
    for update_num, update_dict in enumerate(study_RLalg.all_policies):
        ...
        if args.action_centering:
            ...
            est_eqns_dict = study_RLalg.get_est_eqns_df(beta_est=beta_est,
                                                     data_sofar=data_sofar,
                                                     all_user_ids=all_user_id,
...
def get_adaptive_sandwich_new(all_est_eqn_dict, study_RLalg, study_df, 
    ...
    all_user_id = set( study_df['user_id'].to_numpy() )
    
    all_estimators = []
    policy2collected = {}
    update2esteqn = {}
    # `study_RLalg.all_policies` includes a ``final policy'' that updates after the study concludes
    ...
    for update_num, update_dict in enumerate(study_RLalg.all_policies):
        ...
        if update_num != len(study_RLalg.all_policies):
            if args.RL_alg == "sigmoid_LS":
        ...
        if policy_num > 1:  # we do not form weights for data prior to the first update
            ...
            if args.RL_alg == 'posterior_sampling':
                tmp_collected['intercept_val'] = update_dict['intercept_val']
                tmp_collected['n_users'] = update_dict['n_users']
            policy2collected[policy_num] = tmp_collected

    thetahat = all_est_eqn_dict['estimator']
    all_estimators.append( thetahat )
    ...
    info_dict = {
            ...
            "study_df": study_df,
            "study_RLalg": study_RLalg,
            }
    if args.RL_alg == 'posterior_sampling':
        info_dict['prior_dict'] = study_RLalg.prior_dict


    """
    # Preprocess some policy data ##########################
    for update_num, curr_policy_dict in enumerate(study_RLalg.all_policies):
        ...
        if update_num == 1:
            ...
            for key in curr_policy_dict['inc_data'].keys():
                all_data_dict[key] = curr_policy_dict['inc_data'][key].copy()
                curr_policy_dict['all_prev_data'][key] = all_data_dict[key].copy()
            #print(update_num, curr_policy_dict['all_prev_data']['rewards'].shape)
        else:
            curr_policy_dict['all_prev_data'] = {}
            for key in curr_policy_dict['inc_data'].keys():
                new_data = curr_policy_dict['inc_data'][key].copy()
