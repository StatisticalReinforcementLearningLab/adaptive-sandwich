...
class SigmoidLS:
    ...
    def get_weights(self, beta_est, collected_data_dict):
        """
        ...
        add_users = collected_data_dict['all_user_id'] - \
                            collected_data_dict['unique_user_id']
        if len(add_users) > 0:
    ...
    def prep_algdata(self, study_df):
        """
        Prepare algorithm data statistics to form adaptive sandwich variance estimate

        Inputs:
        - `study_df`: study data pandas dataframe 

        Outputs: A dictionary with the following keys and values
        - `all_estimators`: concatenated vector with algorithm statistics (betahats) and inferential target estimator (thetahat); vector is of dimension num_updates*beta_dim + theta_dim
