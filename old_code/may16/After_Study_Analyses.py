...
from least_squares_helper import fit_WLS
...
def get_est_eqn_LS(outcome_vec, design, present_user_ids, est_param, avail_vec,
                   ...
                   reconstruct_check=False, RL_alg=None, intercept_val=None, light=False):
    """
    ...
    if prior_dict is not None:
        ...
        post_mean_adj = -np.matmul(prior_dict['inv_prior_var'], est_param-prior_dict["prior_mean"]) / n_unique
    ...
    if len(add_users) > 0:
        all_user_ids = np.concatenate( [[x for x in add_users], present_user_ids] )
        ...
        all_user_ids = all_user_ids[sort_idx]
        raw_est_eqns = raw_est_eqns[sort_idx]
    
    est_eqns = npi.group_by(present_user_ids).sum(raw_est_eqns)[1]
    if prior_dict is not None:
        est_eqns = est_eqns + post_adj
    #toc = time.perf_counter()
    #print(f"Downloaded the tutorial in {toc - tic:0.4f} seconds")
...
def get_est_eqns(self, beta_params, data_dict, all_user_ids,
        ...
        prior_dict = {
                ...
                "inv_prior_var": self.inv_prior_var,
...
def get_weights(self, beta_params, all_user_ids,
        ...
        prob1_beta = get_action_probs_inner(self, treat_states, beta_params,
...
def generalized_logistic2(args, lin_est):
    # https://timvieira.github.io/blog/post/2014/02/11/exp-normalize-trick/
    inner = args.steepness*lin_est/args.allocation_sigma
    pos_mask = inner >= 0
    exp_inner = np.exp(-inner)
    raw = 1 / (1 + exp_inner) * pos_mask + exp_inner / (1 + exp_inner) * (1-pos_mask)
    pis = args.lower_clip + (args.upper_clip-args.lower_clip) * raw
    return pis
...
def get_action_probs_inner(self, treat_states, beta_est, n_users,
        ...
        #prob_samples2 = generalized_logistic2(self.args, post_samples)
...
def get_stacked_estimating_function(all_beta_params, beta_dim, theta_dim,
                                    study_RLalg, all_collected_data_dict, 
                                    all_user_ids, study_df, return_full=False,
    ...
    for policy_num, curr_policy_dict in enumerate(study_RLalg.all_policies):
        ...
        if args.action_centering:
            ...
            est_eqns_dict = study_RLalg.get_est_eqns_df(beta_params=beta_est,
            ...
            est_eqns_dict = get_est_eqns(study_RLalg, beta_params=beta_est,
                                                     data_dict=curr_policy_dict['all_prev_data'],
                                                     data_sofar=None, #data_sofar, # TODO remove
                                                     all_user_ids=all_user_ids,
                                                     correction=alg_correction,
                                                     check=check,
                                                     intercept_val=intercept_val,
        ...
        #user_pi_weights1 = study_RLalg.get_weights_old(curr_policy_decision_data, 
        #                        beta_est, all_user_ids=all_user_ids,
        #                        intercept_val = intercept_val,
        #                        filter_keyval = ('policy_last_t', policy_last_t) )
       
        #curr_policy_dict['all_prev_data'],
        #user_pi_weights = study_RLalg.get_weights( beta_est, 
        user_pi_weights = get_weights( study_RLalg, beta_est, 
                    collected_data_dict = all_collected_data_dict[policy_last_t],
                    curr_policy_decision_data=None, #curr_policy_decision_data,
                    all_user_ids=all_user_ids,
                    intercept_val = intercept_val,
                    filter_keyval = ('policy_last_t', policy_last_t) )
...
def get_adaptive_sandwich_new(all_est_eqn_dict, study_RLalg, study_df, 
    ...
    # TODO add data colleccted with that policy
    # Preprocess some policy data ##########################
    all_collected_data_dict = {}
    all_user_ids = np.unique( study_df['user_id'].to_numpy() )
    ...
    for policy_num, curr_policy_dict in enumerate(study_RLalg.all_policies):
        ...
        curr_policy_decision_data = study_df[ study_df['policy_last_t'] == policy_last_t ]
        base_states, treat_states = study_RLalg.get_states(curr_policy_decision_data)
        collected_data_dict = {
                'actions': curr_policy_decision_data['action'].to_numpy(),
                'action1probs': curr_policy_decision_data['action1prob'].to_numpy(),
                'base_states': base_states,
                'treat_states': treat_states,
                'user_ids': curr_policy_decision_data['user_id'].to_numpy(),
                }
        all_collected_data_dict[policy_num] = collected_data_dict
        
        if policy_num == 1:
            all_data_dict = {}
            curr_policy_dict['all_prev_data'] = {}
            for key in curr_policy_dict['inc_data_dict'].keys():
                all_data_dict[key] = curr_policy_dict['inc_data_dict'][key].copy()
                curr_policy_dict['all_prev_data'][key] = all_data_dict[key].copy()
            #print(policy_num, curr_policy_dict['all_prev_data']['rewards'].shape)
        else:
            curr_policy_dict['all_prev_data'] = {}
            for key in curr_policy_dict['inc_data_dict'].keys():
                new_data = curr_policy_dict['inc_data_dict'][key].copy()
                all_data_dict[key] = np.concatenate( [all_data_dict[key], new_data], axis=0)
                curr_policy_dict['all_prev_data'][key] = all_data_dict[key].copy()
            #print(policy_num, curr_policy_dict['all_prev_data']['rewards'].shape, \
            #        all_data_dict['rewards'].shape)
    ...
    #print("Stacked estimating equation done")
    #import ipdb; ipdb.set_trace()
    ...
    eps_bigger = 1e-3
    tic = time.perf_counter()
    stacked_hessian2 = optimize.approx_fprime(all_est_params, 
                                        get_stacked_estimating_function,
                                        eps_bigger, beta_dim, theta_dim,
                                        study_RLalg, all_collected_data_dict, 
                                        all_user_ids, study_df)
    toc = time.perf_counter()
    print(f"Downloaded the tutorial in {toc - tic:0.4f} seconds")
    ...
    """
    tic = time.perf_counter()
    stacked_hessian2 = nd_statsmodels.approx_fprime( all_est_params, 
                        lambda x : get_stacked_estimating_function(x,
                                        beta_dim, theta_dim,
                                        study_RLalg, study_df),
                        epsilon=eps_bigger)
    toc = time.perf_counter()
