...
class SmoothPosteriorSampling:
    ...
    def get_est_eqns(self, beta_params, data_dict, all_user_ids, 
        ...
        """
        # Posterior Mean and V (using dataframe) #########################
        actions = data_sofar.action.to_numpy().reshape(-1,1)
        base_states = data_sofar[self.state_feats].to_numpy()
        treat_states = data_sofar[self.treat_feats].to_numpy()
        ...
        est_eqn_dict2 = get_est_eqn_LS(outcome_vec, design, user_ids,
                                      beta_params, avail_vec, all_user_ids,
                                      prior_dict = prior_dict,
                                      correction = correction,
                                      reconstruct_check = check,
        ...
        """
        try:
            assert (outcome_vec == data_dict['rewards']).all()
            assert (actions.flatten() == data_dict['actions']).all()
            assert (base_states == data_dict['base_states']).all()
            assert (treat_states == data_dict['treat_states']).all()
            assert (avail_vec == data_dict['avail']).all()
            assert (user_ids == data_dict['user_ids']).all()
        except:
            import ipdb; ipdb.set_trace()
        """
