...
class RLAlgorithm:
    def __init__(self, cost_params, update_cadence, smoothing_func):
        ...
        self.reward_def_func = lambda brushing_quality, current_action, b_bar, a_bar: reward_definition.calculate_reward(
            brushing_quality,
            cost_params[0],
            cost_params[1],
            current_action,
...
def process_alg_state_v1(env_state, b_bar, a_bar):
    baseline_state = np.array(
        [env_state[0], reward_definition.normalize_b_bar(b_bar), a_bar, 1]
    )
...
def process_alg_state_v2(env_state, b_bar, a_bar):
    baseline_state = np.array(
        [
            env_state[0],
            reward_definition.normalize_b_bar(b_bar),
            reward_definition.normalize_a_bar(a_bar),
            env_state[-1],
            1,
        ]
    )
...
def process_alg_state_v3(env_state):
    baseline_state = np.array(
        [env_state[0], env_state[1], env_state[2], env_state[3], 1]
    )
    advantage_state = np.copy(baseline_state)
...
def create_big_phi(advantage_states, baseline_states, actions, probs):
    big_phi = np.hstack(
        (
            baseline_states,
            np.multiply(advantage_states.T, probs).T,
