...
STAT_PARAMS_DF = pd.read_csv(
    read_write_info.READ_PATH_PREFIX + "sim_env_data/v3_stat_zip_model_params.csv"
)
NON_STAT_PARAMS_DF = pd.read_csv(
    read_write_info.READ_PATH_PREFIX + "sim_env_data/v3_non_stat_zip_model_params.csv"
)
APP_OPEN_PROB_DF = pd.read_csv(
    read_write_info.READ_PATH_PREFIX + "sim_env_data/v3_app_open_prob.csv"
)
...
def get_app_open_prob(user_id):
    return APP_OPEN_PROB_DF[APP_OPEN_PROB_DF["user_id"] == user_id][
        "app_open_prob"
    ].values[0]
...
def get_base_params_for_user(user, env_type="STAT"):
    ...
    # poisson parameters, bernoulli parameters
...
def get_user_effect_funcs():
    ...
    def bern_adv_func(state, adv_params):
        return min(adv_params @ state, 0)

    # positive treatment effect means users brush more seconds
    def y_adv_func(state, adv_params):
        return max(adv_params @ state, 0)
...
class SimulationEnvironmentV3(
    ...
    def __init__(self, users_list, env_type, delayed_effect_scale):
        ...
        super().__init__(users_list, user_envs, env_type)
