...
def run_study_simulation(args, study_env, study_RLalg, user_env_data):
    ...
    ######## change: add Z_id column to signal whether it is treatment or control group (pooling algorithm vs MRT)
    if args.Twoarmed == 1:
        study_df['Z_id'] = 0
        study_df.loc[study_df['user_id'] <= int(study_df['user_id'].max()/2), 'Z_id'] = 1 # the half users to the treatment group
    else:
        study_df['Z_id'] = 1
    
    # study_df['Z_id'] = args.Z_id
    # if args.Z_id == 0:
    #     study_df['user_id'] += args.n # control group user_id: n+1, n+2, ..., 2n
...
9        4         1.0       3        1           1          NaN          1     NaN     NaN            0.0     0.408789                   0.0     0.0         1                   9           0.887792          -2.324833     0
        ...
        all_prev_data_bool = study_df["calendar_t"] <= t # all historical data
...
def load_data_and_simulate_studies(args, gen_feats, alg_state_feats, alg_treat_feats):
    ...
    for i in range(1, args.N + 1):
        ...
        toc1 = toc2
        print("env_seed", env_seed, "alg_seed", alg_seed)
        ...
        if args.RL_alg == RLStudyArgs.FIXED_RANDOMIZATION:
            ...
            study_RLalg = SmoothPosteriorSampling(
                ...
                upper_clip=args.upper_clip,
                steepness=args.steepness,
                action_centering=args.action_centering, # whether the action centering is used
                prior_mu=prior_mean,
            ...
            study_RLalg = SoftActorCritic(
                ...
                steepness=args.steepness,
                twoarmed=bool(args.Twoarmed), # Twoarmed=1, update for Z_id=1, and no update (fixed randomization)for Z_id=0 
                history=bool(args.SAChistory) # history=0: vanilla SAC; history=1: historical SAC 
            )
        ...
        # add_name = "_random" if args.Z_id == 0 else ""
        add_name = ""

        print(f"Saving to {folder_path}!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        study_df.to_csv(f"{folder_path}/data{add_name}.csv", index=False)
        with open(f"{folder_path}/study_df{add_name}.pkl", "wb") as f:
            pickle.dump(study_df, f)

        # save all information of RL's algorithm
        with open(f"{folder_path}/study_RLalg{add_name}.pkl", "wb") as f:
            pickle.dump(study_RLalg, f)

        # save the policy's parameters
        with open(f"{folder_path}/pi_args{add_name}.pkl", "wb") as f:
            pickle.dump(study_RLalg.pi_args, f)


        with open(f"{folder_path}/rl_update_args{add_name}.pkl", "wb") as f:
...
def main():
    ...
    parser.add_argument("--n", type=int, default=6, help="Total number of users") ############ important ############
    parser.add_argument(
        "--upper_clip",
        type=float,
        default=0.9,
    ...
    parser.add_argument(
        "--save_dir", type=str, default="./n30_T50/0", help="Directory to save all results in"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="filename for saving the data (optional)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="coefficient for the 1st pre-treatment feature",
    )
