...
# sbatch --array=[0-99] run_and_analysis_parallel_oralytics.sh --T=25 --n=100 --recruit_n=100 --recruit_t=1
...
cd ~
if ! test -d venv; then
  echo $(date +"%Y-%m-%d %T") run_and_analysis_parallel_oralytics.sh: Creating venv, as it did not exist.
  python3 -m venv venv
fi
...
if [ "$only_analysis" -eq "0" ]; then
  echo "$(date +"%Y-%m-%d %T") run_and_analysis_parallel_oralytics: Beginning RL study simulation."
...
echo "$(date +"%Y-%m-%d %T") run_and_analysis_parallel_oralytics.sh: When all jobs have completed, you may collect and summarize the analyses with: bash simulation_collect_analyses.sh --input_glob=${output_folder_glob}/analysis.pkl --num_users=$num_users --index_to_check_ci_coverage=6"
