...
class SmoothPosteriorSampling:
    ...
    def __init__(
        ...
        norm_samples = self.rng.normal(
            loc=0, scale=1, size=(args.T * args.n, NUM_POSTERIOR_SAMPLES)
        )
    ...
    def update_alg(self, new_data, update_last_t):
        ...
        if self.action_centering:
            design = np.concatenate(
                [
                    base_states,
                    action1prob * treat_states,
                    (actions - action1prob) * treat_states,
                ],
                axis=1,
            )
        else:
    ...
    def get_action_probs(self, curr_timestep_data, filter_keyval):
        ...
        probs = self.get_action_probs_inner(
            beta_est, prob_input_dict, check_post_var=post_var
        )
    ...
    def get_action_probs_inner(self, beta_est, prob_input_dict, check_post_var=None):
        ...
        post_var_user = np.einsum(
            "ij,ij->i", np.matmul(treat_states, post_var_treat), treat_states
        )
        ...
        try:
            ...
            post_samples = post_mean_user.reshape(n_tmp, -1) + prob_input_dict[
                "norm_samples"
    ...
    def prep_algdata(self):
        ...
        for update_num, update_dict in enumerate(self.all_policies):
            ...
            if update_num != len(self.all_policies):
                ...
                update2esteqn[update_num]["intercept_val"] = update_dict[
                    "intercept_val"
                ]
