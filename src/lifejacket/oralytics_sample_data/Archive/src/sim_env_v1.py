...
DATA_DF = pd.read_csv(
    "https://raw.githubusercontent.com/ROBAS-UCLA/ROBAS.3/main/data/robas_3_data_complete.csv"
)
...
def get_user_sessions(user_id, env_type):
    return (
        USERS_SESSIONS_STAT[user_id]
        if env_type == "STAT"
        else USERS_SESSIONS_NON_STAT[user_id]
    )
...
def create_user_envs(users_list, effect_size_scale, delayed_effect_scale_val, env_type):
    ...
    for i, user_id in enumerate(users_list):
        ...
        new_user = simulation_environment.UserEnvironment(
            user_id,
            model_type,
            user_sessions,
            user_effect_sizes,
