...
STAT_PARAMS_DF = pd.read_csv(
    read_write_info.READ_PATH_PREFIX + "sim_env_data/stat_user_models.csv"
)
NON_STAT_PARAMS_DF = pd.read_csv(
    read_write_info.READ_PATH_PREFIX + "sim_env_data/non_stat_user_models.csv"
)
...
def generate_env_state(
    j, brushing_qualities, app_engagement, env_type, per_user_weeks_in_study=10
):
    ...
    previous_day_total_rewards = get_previous_day_total_brush_quality(
        brushing_qualities, session_type, j
    )
    env_state[1] = simulation_environment.normalize_total_brush_quality(
        previous_day_total_rewards
    )
    if env_type == "STAT":
        ...
        env_state[2] = simulation_environment.normalize_day_in_study(
...
with open(
    read_write_info.READ_PATH_PREFIX + "sim_env_data/smaller_stat_user_effect_sizes.p",
    "rb",
) as f:
    SMALLER_STAT_USER_EFFECT_SIZES = pickle.load(f)
with open(
    read_write_info.READ_PATH_PREFIX
    + "sim_env_data/smaller_non_stat_user_effect_sizes.p",
    "rb",
) as f:
    SMALLER_NON_STAT_USER_EFFECT_SIZES = pickle.load(f)
with open(
    read_write_info.READ_PATH_PREFIX
    + "sim_env_data/less_small_stat_user_effect_sizes.p",
    "rb",
) as f:
    LESS_SMALL_STAT_USER_EFFECT_SIZES = pickle.load(f)
with open(
    read_write_info.READ_PATH_PREFIX
    + "sim_env_data/less_small_non_stat_user_effect_sizes.p",
    "rb",
) as f:
...
stat_user_spec_effect_func_bern = lambda state, effect_sizes: -1.0 * max(
    effect_sizes @ np.delete(state, 2), 0
)
stat_user_spec_effect_func_y = lambda state, effect_sizes: max(
    effect_sizes @ np.delete(state, 2), 0
)
non_stat_user_spec_effect_func_bern = lambda state, effect_sizes: -1.0 * max(
    effect_sizes @ np.delete(state, 3), 0
)
non_stat_user_spec_effect_func_y = lambda state, effect_sizes: max(
    effect_sizes @ np.delete(state, 3), 0
)
...
class UserEnvironmentV2(simulation_environment.UserEnvironmentAppEngagement):
    def __init__(
        ...
        super(UserEnvironmentV2, self).__init__(
            user_id,
            model_type,
            user_effect_sizes,
            delayed_effect_scale_val,
...
class SimulationEnvironmentV2(
    simulation_environment.SimulationEnvironmentAppEngagement
):
    def __init__(self, users_list, env_type, effect_size_scale, delayed_effect_scale):
        delayed_effect_scale_val = simulation_environment.get_delayed_effect_scale(
            delayed_effect_scale
        )
        user_envs = create_user_envs(
            users_list, effect_size_scale, delayed_effect_scale_val, env_type
        )
    ...
    def generate_current_state(self, user_idx, j, per_user_weeks_in_study):
        ...
        return generate_env_state(
            j, brushing_qualities, prior_app_engagement, self.get_env_type()
        )
